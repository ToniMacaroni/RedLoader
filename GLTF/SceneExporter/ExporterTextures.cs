...
namespace UnityGLTF
{
	public partial class GLTFSceneExporter
	{
		...
		private void ExportImages(string outputPath)
		{
			...
			{
				GLTFSceneExporter.writeImageToDiskMarker.Begin();
				...
				if (!wasAbleToExportTexture)
				{
					...
					{
						case GLTFSceneExporter.TextureExportSettings.Conversion.MetalGlossChannelSwap:
							GetConversionMaterial(textureMapType); // Sets paramaters to the material
							ExportLinearTexture(image, fileOutputPath, true);
							break;
						case GLTFSceneExporter.TextureExportSettings.Conversion.MetalGlossOcclusionChannelSwap:
							GetConversionMaterial(textureMapType); // Sets paramaters to the material
							ExportLinearTexture(image, fileOutputPath, true, true);
							break;
						case GLTFSceneExporter.TextureExportSettings.Conversion.None:
							if (textureMapType.linear)
								ExportLinearTexture(image, fileOutputPath, false);
							else
								ExportTexture((Texture2D)image, fileOutputPath);
							break;
						case GLTFSceneExporter.TextureExportSettings.Conversion.NormalChannel:
				...
				GLTFSceneExporter.writeImageToDiskMarker.End();
			}
		}
		...
		{
			UnityEngine.Debug.LogWarning("Exporting textures not supported yet");
			return;
			
			if (!texture)
			{
				UnityEngine.Debug.Log("Can not export missing texture: " + outputPath);
				return;
			}
		...
		private void WriteRenderTextureToDiskAndRelease(RenderTexture destRenderTexture, string outputPath, bool linear)
		{
			...
			{
				UnityEngine.Debug.LogError("Unsupported file extension: " + outputPath, destRenderTexture);
				binaryData = exportTexture.EncodeToPNG();
			}
		...
		public TextureInfo ExportTextureInfo(Texture texture, string textureSlot, GLTFSceneExporter.TextureExportSettings exportSettings = default)
		{
		...
		public TextureId ExportTexture(Texture textureObj, string textureSlot, GLTFSceneExporter.TextureExportSettings exportSettings = default)
		{
			var uniqueTexture = exportSettings.isValid ?
				new GLTFSceneExporter.UniqueTexture(textureObj, exportSettings) :
				new GLTFSceneExporter.UniqueTexture(textureObj, textureSlot, this);
			...
			if (textureSlot == GLTFSceneExporter.TextureMapType.Custom_HDR)
			{
		...
		private string GetImageOutputPath(Texture texture, GLTFSceneExporter.TextureExportSettings textureMapType, string textureSlot, out bool ableToExportFromDisk)
		{
			...
			{
				case GLTFSceneExporter.TextureExportSettings.AlphaMode.Never:
					textureHasAlpha = false;
					break;
				case GLTFSceneExporter.TextureExportSettings.AlphaMode.Heuristic:
			...
			{
				imagePath = Path.GetFileName((string)imagePath);
			}
		...
		private ImageId ExportImage(GLTFSceneExporter.UniqueTexture uniqueTexture, string textureSlot)
		{
            ...
            _imageInfos.Add(new GLTFSceneExporter.ImageInfo
			{
		...
		private bool CanGetTextureDataFromDisk(GLTFSceneExporter.TextureExportSettings textureMapType, Texture texture, out string path)
		{
			path = null;
		...
		private ImageId ExportImageInternalBuffer(GLTFSceneExporter.UniqueTexture uniqueTexture, string textureSlot)
		{
			var texture = uniqueTexture.Texture;
			...
			if (settings.TryExportTexturesFromDisk && CanGetTextureDataFromDisk(exportSettings, texture, out string path))
			{
				...
				{
					UnityEngine.Debug.Log("Texture can't be exported from disk: " + path + ". Only PNG & JPEG are supported. The texture will be re-encoded as PNG.", texture);
				}
			}

			// export in-memory floating point textures as EXR
			// TODO add readback when not readable
			if (!wasAbleToExport && textureSlot == GLTFSceneExporter.TextureMapType.Custom_HDR)
			{
			...
			if (!wasAbleToExport)
		    {
				...
				switch (exportSettings.alphaMode)
				{
					case GLTFSceneExporter.TextureExportSettings.AlphaMode.Never:
						textureHasAlpha = false;
						break;
					case GLTFSceneExporter.TextureExportSettings.AlphaMode.Always:
				...
				if (textureSlot == GLTFSceneExporter.TextureMapType.Custom_HDR)
				{
					UnityEngine.Debug.LogWarning("HDR Texture Export from non-readable textures isn't supported yet", texture);
				}
			...
			// {
			// 	UnityEngine.Debug.LogWarning("Validation Warning: " + "Image has non-power-of-two dimensions: " + texture.width + "x" + texture.height + ".", texture);
			// }
		...
		private SamplerId ExportSampler(Texture texture)
		{
			...
			switch (texture.wrapMode)
			{
				...
				case TextureWrapMode.Mirror:
					...
					break;
				default:
					UnityEngine.Debug.LogWarning("Unsupported Texture.wrapMode: " + texture.wrapMode, texture);
					sampler.WrapS = WrapMode.Repeat;
					sampler.WrapT = WrapMode.Repeat;
					break;
			}
			...
			if(dataForTexture.HasMipmaps)
			{
				switch (texture.filterMode)
				{
					...
					case FilterMode.Trilinear:
						...
						break;
					default:
						UnityEngine.Debug.LogWarning("Unsupported Texture.filterMode: " + texture.filterMode, texture);
						sampler.MinFilter = MinFilterMode.LinearMipmapLinear;
						sampler.MagFilter = MagFilterMode.Linear;
						break;
				}
