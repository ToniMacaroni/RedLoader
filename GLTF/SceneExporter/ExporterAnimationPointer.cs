...
#region Shader Property Names
		...
		public void AddAnimationData(Object animatedObject, string propertyName, GLTFAnimation animation, float[] times, object[] values)
		{
			...
			for (var i = 0; i < values.Length; i++)
			{
				...
				UnityEngine.Debug.LogError($"GLTFExporter error: value {i} in animated property \"{propertyName}\" is null. Skipping", animatedObject);
				return;
			}
			...
			{
				UnityEngine.Debug.LogWarning($"Animation for {animatedObject.name} ({animatedObject.GetType()}) has not been exported as the object itself is not exported (disabled/EditorOnly). (InstanceID: {animatedObject.GetInstanceID()})", animatedObject);
				return;
			}
			...
			switch (animatedObject)
			{
				case Material material:
					...
					switch (propertyName)
					{
						...
						// 	break;
						default:
							GLTFSceneExporter.Debug.Log(LogType.Warning, "Unknown property name on Material " + material + ": " + propertyName);
							break;
					}
					break;
				case Light light:
					...
					switch (propertyName)
					{
						...
						default:
							extensionName = null;
							GLTFSceneExporter.Debug.Log(LogType.Warning, "Unknown property name on Light " + light + ": " + propertyName);
							break;
					}
					break;
				case Camera camera:
					if(camera.orthographic)
					{
						switch (propertyName)
						{
							...
							case "far clip plane":
								...
								break;
							default:
								GLTFSceneExporter.Debug.Log(LogType.Warning, "Unknown property name on Camera " + camera + ": " + propertyName);
								break;
						}
					}
					else
				...
				case SkinnedMeshRenderer skinnedMesh:
					...
					if (!sharedMesh)
					{
						GLTFSceneExporter.Debug.Log(LogType.Error, "No mesh on SkinnedMeshRenderer " + skinnedMesh + ", skipping animation");
						return;
					}
			...
			GLTF.Schema.AnimationSampler Tsampler = new GLTF.Schema.AnimationSampler();
			...
			GLTF.Schema.AnimationSampler Tsampler2 = new GLTF.Schema.AnimationSampler();
			...
			switch (val)
			{
				...
				case float[] _:
					...
					for (var i = 1; i < values.Length; i++)
					{
						...
						GLTFSceneExporter.Debug.Log(LogType.Error, "When animating float arrays, all array entries must have the same length. Skipping");
						return;
					}
				...
				case Color _:
					if (propertyName == "emissiveFactor" && secondPropertyName != null)
					{
						...
						var pluginSettings = (Enumerable.FirstOrDefault<GLTFExportPluginContext>(_plugins, x => x is MaterialExtensionsExportContext) as MaterialExtensionsExportContext)?.settings;
			...
			{
				GLTFSceneExporter.Debug.LogError($"GLTFExporter: Something went wrong, empty sampler output for \"{propertyName}\" in {animatedObject}", animatedObject);
				return;
			}
