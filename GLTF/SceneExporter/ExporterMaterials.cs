...
namespace UnityGLTF
{
	public partial class GLTFSceneExporter
	{
		...
		public static event GLTFSceneExporter.BeforeSceneExportDelegate BeforeSceneExport;
		[Obsolete("Use ExportPlugins on GLTFSettings instead")]
		public static event GLTFSceneExporter.AfterSceneExportDelegate AfterSceneExport;
		[Obsolete("Use ExportPlugins on GLTFSettings instead")]
		public static event GLTFSceneExporter.AfterNodeExportDelegate AfterNodeExport;
		/// <returns>True: material export is complete. False: continue regular export.</returns>
		[Obsolete("Use ExportPlugins on GLTFSettings instead")]
		public static event GLTFSceneExporter.BeforeMaterialExportDelegate BeforeMaterialExport;
		[Obsolete("Use ExportPlugins on GLTFSettings instead")]
		public static event GLTFSceneExporter.AfterMaterialExportDelegate AfterMaterialExport;
        ...
        public MaterialId ExportMaterial(Material materialObj)
		{
			...
			foreach (var plugin in _plugins)
			{
				...
				GLTFSceneExporter.beforeMaterialExportMarker.Begin();
				if (plugin.BeforeMaterialExport(this, _root, materialObj, material))
				{
					GLTFSceneExporter.beforeMaterialExportMarker.End();
					return CreateAndAddMaterialId(materialObj, material);
				}
				else
				{
			...
			GLTFSceneExporter.exportMaterialMarker.Begin();
			var isBirp =
#if UNITY_2019_3_OR_NEWER
			...
			if (materialObj.IsKeywordEnabled("_EMISSION") || materialObj.IsKeywordEnabled("EMISSION") || materialObj.HasProperty("emissiveTexture") || materialObj.HasProperty("_EmissiveTexture"))
			{
				...
				if (materialObj.HasProperty("_EmissionColor") || materialObj.HasProperty("emissiveFactor") || materialObj.HasProperty("_EmissiveFactor"))
				{
					...
					{
						var materialSettings = (Enumerable.FirstOrDefault<GLTFExportPluginContext>(_plugins, x => x is MaterialExtensionsExportContext) as MaterialExtensionsExportContext)?.settings;
						var emissiveStrengthSupported = materialSettings != null && materialSettings.KHR_materials_emissive_strength;
				...
				if (materialObj.HasProperty("_EmissionMap") || materialObj.HasProperty("_EmissiveMap") || materialObj.HasProperty("_EmissiveTexture") || materialObj.HasProperty("emissiveTexture"))
				{
					...
					if (emissionTex)
					{
						...
						{
							material.EmissiveTexture = ExportTextureInfo(emissionTex, GLTFSceneExporter.TextureMapType.Emissive);

							ExportTextureTransform(material.EmissiveTexture, materialObj, propName);
						}
						else
						{
							UnityEngine.Debug.LogError($"Can't export a {emissionTex.GetType()} emissive texture in material {materialObj.name}");
						}
			...
			if (materialObj.HasProperty("_BumpMap") && (materialObj.IsKeywordEnabled("_NORMALMAP") || materialObj.IsKeywordEnabled("_BUMPMAP") || is_glTFastShader))
			{
				...
				if (normalTex)
				{
					...
					{
						material.NormalTexture = ExportNormalTextureInfo(normalTex, GLTFSceneExporter.TextureMapType.Normal, materialObj);
						ExportTextureTransform(material.NormalTexture, materialObj, "_BumpMap");
					}
					else
					{
						UnityEngine.Debug.LogError(string.Format("Can't export a {0} normal texture in material {1}", normalTex.GetType(), materialObj.name));
					}
				}
			}
			...
			else if (materialObj.HasProperty("_BaseMap"))
			{
				...
				material.PbrMetallicRoughness = new PbrMetallicRoughness()
				{
					BaseColorFactor = (materialObj.HasProperty("_BaseColor")
						...
						: UnityEngine.Color.white).ToNumericsColorLinear(),
					BaseColorTexture = mainTex ? ExportTextureInfo(mainTex, GLTFSceneExporter.TextureMapType.BaseColor) : null
				};
            ...
            else if (materialObj.HasProperty("_MainTex")) //else export main texture
            {
                ...
                {
                    material.PbrMetallicRoughness = new PbrMetallicRoughness() { MetallicFactor = 0, RoughnessFactor = 1.0f };
                    material.PbrMetallicRoughness.BaseColorTexture = ExportTextureInfo(mainTex, GLTFSceneExporter.TextureMapType.BaseColor);
			...
			if (materialObj.HasProperty("_OcclusionMap") || materialObj.HasProperty("occlusionTexture") || materialObj.HasProperty("_OcclusionTexture"))
			{
				...
				if (occTex)
				{
					if(occTex is Texture2D)
					{
						...
						material.OcclusionTexture = ExportOcclusionTextureInfo(occTex, GLTFSceneExporter.TextureMapType.Occlusion, materialObj, sharedTextureId);
					...
					{
						UnityEngine.Debug.LogError($"Can't export a {occTex.GetType()} occlusion texture in material {materialObj.name}");
					}
				}
			}
			GLTFSceneExporter.exportMaterialMarker.End();
        ...
        private MaterialId CreateAndAddMaterialId(Material materialObj, GLTFMaterial material)
        {
	        ...
	        if (materialObj)
	        {
		        GLTFSceneExporter.afterMaterialExportMarker.Begin();
		        foreach (var plugin in _plugins)
			        plugin?.AfterMaterialExport(this, _root, materialObj, material);
		        GLTFSceneExporter.afterMaterialExportMarker.End();
	        }
		...
		private void ExportTextureTransform(TextureInfo def, Material mat, string texName)
		{
			...
			if (offset == Vector2.zero && scale == Vector2.one && rotation == 0)
			{
				...
				// UnityEngine.Debug.Log("Checking for property: " + checkForName + " : " + mat.HasProperty(checkForName) + " == " + (mat.HasProperty(checkForName) ? mat.GetVector(checkForName) : "null"));
			...
			Material material)
		{
			...
			GLTFSceneExporter.TextureExportSettings exportSettings = default;
			...
			if (material.GetFloat(normalMapFormatIsXYZ) >= 1)
			{
				...
				exportSettings.alphaMode = GLTFSceneExporter.TextureExportSettings.AlphaMode.Never;
				exportSettings.conversion = GLTFSceneExporter.TextureExportSettings.Conversion.None;
			}
		...
		public PbrMetallicRoughness ExportPBRMetallicRoughness(Material material)
		{
            ...
            if (material.HasProperty("_MainTex") || material.HasProperty("_BaseMap") || material.HasProperty("_BaseColorTexture") || material.HasProperty("baseColorTexture")) //TODO if additive particle, render black into alpha
			{
				...
				if (mainTex)
				{
					pbr.BaseColorTexture = ExportTextureInfo(mainTex, GLTFSceneExporter.TextureMapType.BaseColor);
			...
			if (material.HasProperty("metallicRoughnessTexture"))
			{
				...
				if (mrTex)
				{
					pbr.MetallicRoughnessTexture = ExportTextureInfo(mrTex, GLTFSceneExporter.TextureMapType.MetallicRoughness);
			...
			else if (material.HasProperty("_MetallicGlossMap"))
			{
				...
				if (mrTex)
				{
					...
					var conversion = GetExportSettingsForSlot((isGltfPbrMetallicRoughnessShader || isGlTFastShader) ? GLTFSceneExporter.TextureMapType.Linear : GLTFSceneExporter.TextureMapType.MetallicGloss);
					if (needToBakeRoughnessIntoTexture)
					{
						conversion = new GLTFSceneExporter.TextureExportSettings(conversion);
						conversion.smoothnessMultiplier = 1 - roughnessMultiplier;
					}
					
					if (occlusionGetBakedIntoMetallicRoughness)
					{
						conversion.conversion = GLTFSceneExporter.TextureExportSettings.Conversion.MetalGlossOcclusionChannelSwap;
						_occlusionBakedTextures.Add(material);
					}
					
					pbr.MetallicRoughnessTexture = ExportTextureInfo(mrTex, GLTFSceneExporter.TextureMapType.MetallicRoughness, conversion);
		...
		private void ExportPBRSpecularGlossiness(GLTFMaterial material, Material materialObj)
		{
			...
			Color diffuseFactor = KHR_materials_pbrSpecularGlossinessExtension.DIFFUSE_FACTOR_DEFAULT;
			...
			if (materialObj.HasProperty("_BaseMap"))
			{
				...
				if (mainTex != null)
				{
					diffuseTexture = ExportTextureInfo(mainTex, GLTFSceneExporter.TextureMapType.BaseColor);
			...
			if (materialObj.HasProperty("_SpecColor"))
			{
				...
				if (specGlossMap == null)
				{
					...
					specularFactor = new Vector3(specColor.R, specColor.G, specColor.B);
				}
			}
			...
			if (materialObj.HasProperty("_SpecGlossMap"))
			{
				...
				if (mgTex)
				{
					specularGlossinessTexture = ExportTextureInfo(mgTex, GLTFSceneExporter.TextureMapType.SpecGloss);
		...
		private MaterialCommonConstant ExportCommonConstant(Material materialObj)
		{
			...
			if (materialObj.HasProperty("_LightMap"))
			{
				...
				if (lmTex)
				{
					constant.LightmapTexture = ExportTextureInfo(lmTex, GLTFSceneExporter.TextureMapType.Linear);
