...
namespace UnityGLTF
{
	public partial class GLTFSceneExporter
	{
		...
		{
			GLTFSceneExporter.exportSkinFromNodeMarker.Begin();

			var go = transform.gameObject;
			var skin = transform.GetComponent<SkinnedMeshRenderer>();
			var mesh = GetMeshFromGameObject(go);
			GLTFSceneExporter.UniquePrimitive key = new GLTFSceneExporter.UniquePrimitive();
			key.Mesh = mesh;
			...
			{
				UnityEngine.Debug.Log("No mesh found for skin on " + transform, transform);
				GLTFSceneExporter.exportSkinFromNodeMarker.End();
				return;
			}
			Skin gltfSkin = new Skin();
			...
			for (int i = 0; i < skin.bones.Length; ++i)
			{
				if (!skin.bones[i])
				{
					UnityEngine.Debug.LogWarning("Skin has null bone at index " + i + ": " + skin, skin);
					continue;
				}
			...
			{
				UnityEngine.Debug.LogWarning("Not all bones for SkinnedMeshRenderer " + transform + " were exported. Skin information will be skipped. Make sure the bones are active and enabled if you want to export them.", transform);
				GLTFSceneExporter.exportSkinFromNodeMarker.End();
				return;
			}
			...
			if(val != null)
			{
				GLTFMesh gltfMesh = _root.Meshes[val.Id];
				if(gltfMesh != null)
				{
