...
#pragma warning restore CS0618 // Type or member is obsolete
	...
	internal class ComputeShaderInfo
	{
	...
	public partial class GLTFSceneImporter : IDisposable
	{
		...
		private static ComputeShaderInfo _cachedHdrpComputeShader;

		internal static ComputeShaderInfo HdrpComputeShader
		{
			get
			{
				if (_cachedHdrpComputeShader != null)
					return _cachedHdrpComputeShader;

				var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("GLTF.Resources.gltfbundle");
				if(stream == null)
					throw new Exception("Failed to GLTF Bundle from Assembly. Stream is null.");
        
				var bytes = new byte[stream.Length];
				_ = stream.Read(bytes, 0, bytes.Length);

				var bundle = AssetBundle.LoadFromMemory(bytes);
				var shader = bundle.LoadAsset("gltfhdrp").Cast<ComputeShader>();
				_cachedHdrpComputeShader = new()
				{
					ComputeShader = shader, Kernel = shader.FindKernel("CSMain")
				};
		...
		protected virtual async Task ConstructNode(Node node, int nodeIndex, CancellationToken cancellationToken)
		{
			...
			async Task CreateNodeComponentsAndChilds(bool ignoreMesh = false, bool onlyMesh = false)
			{
				...
				if (!ignoreMesh && node.Mesh != null)
				{
					...
					{
						if (node.Name.StartsWith("collider"))
						{
							var col = nodeObj.AddComponent<BoxCollider>();
							col.center = unityMesh.bounds.center;
							col.size = unityMesh.bounds.size;
							if(DebugLogging) RLog.Msg(System.Drawing.Color.Pink, $"Found and applied collider {nodeObj.name}");
						}
						else
						{
