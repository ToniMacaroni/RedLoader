...
// 				}
			...
			if (sonsMapper != null && ProcessMaterials)
			{
				...
				if (metalRoughMap)
				{
					var pixels = metalRoughMap.GetPixels();
					for (int i = 0; i < pixels.Length; i++)
					{
						var c = pixels[i];
						pixels[i] = new UnityEngine.Color(c.b, 1.0f, 1.0f, 1.0f - c.g);
					}
					
					metalRoughMap.SetPixels(pixels);
					metalRoughMap.Apply();
				...
				if (normalTex)
				{
					var pixels = normalTex.GetPixels();
					for (var i = 0; i < pixels.Length; i++)
					{
						var c = pixels[i];
						pixels[i] = new UnityEngine.Color(1, c.g, 1, c.r);
					}
				...
				sw.Stop();
				if(DebugLogging) RLog.Msg(Color.Orange, $"Converted maps in {sw.ElapsedMilliseconds}");
				
				// if (def.PbrMetallicRoughness.MetallicRoughnessTexture != null)
				// {
				// 	var textureId = def.PbrMetallicRoughness.MetallicRoughnessTexture.Index;
				// 	await ConstructTexture(textureId.Value, textureId.Id, !KeepCPUCopyOfTexture, true, false);
				// 	sonsMapper.MaskMap = _assetCache.TextureCache[textureId.Id].Texture;
				// }
				//
				// if (def.NormalTexture != null)
				// {
				// 	sonsMapper.MaskMap = _assetCache.TextureCache[def.NormalTexture.Index.Id].Texture;
				// }
			}
