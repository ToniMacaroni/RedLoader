...
namespace UnityGLTF
{
	public partial class GLTFSceneImporter
	{
		...
		protected void PrepareUnityMeshData()
		{
			...
			for (int i = 0; i < Enumerable.Count<GLTFMesh>(_gltfRoot.Meshes); i++)
			{
				int meshIndex = i;
...
#pragma warning restore 0219
		...
		protected virtual unsafe void ConstructMeshTargets(MeshPrimitive primitive, int meshIndex, int primitiveIndex)
		{
			...
			for (int i = 0; i < primitive.Targets.Count; i++)
			{
				...
				foreach (var targetAttribute in target)
				{
					...
					var bufferPointer1 = bufferViewCache1.GetUnsafePtrAlt();
					...
					var bufferPointer2 = bufferViewCache2.GetUnsafePtrAlt();
					...
					switch (targetAttribute.Key)
					{
						case NormalKey:
							...
							Accessor.AsSparseFloat3Array(targetAttribute.Value.Value, ref sparseNormals[0],
								bufferPointer1, 0, targetAttribute.Value.Value.Normalized);
						...
						case PositionKey:
							...
							if (hasScale)
								Accessor.AsSparseFloat3ArrayConversion(targetAttribute.Value.Value, ref sparsePositions[0],
									bufferPointer1, scaleFactor, 0, targetAttribute.Value.Value.Normalized);
							else
