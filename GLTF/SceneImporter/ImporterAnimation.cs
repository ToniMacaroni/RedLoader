	...
	public partial class GLTFSceneImporter
	{
			...
			ValuesConvertion getConvertedValues)
		{
			...
			for (var ci = 0; ci < channelCount; ci++)
			{
				...
				if (clip.legacy)
#endif 
					clip.SetCurve(relativePath, Il2CppType.From(curveType), propertyNames[ci], curve);
			}
		}
		...
		private static float GetCurveKeyframeLeftLinearSlope(Keyframe[] keyframes, int keyframeIndex)
		{
			...
			if (timeDelta <= 0)
			{
				...
				GLTFSceneImporter.Debug.Log(LogType.Warning, "Time of subsequent animation keyframes is not increasing (glTF-Validator error ACCESSOR_ANIMATION_INPUT_NON_INCREASING)");
		...
		protected async Task<AnimationClip> ConstructClip(Transform root, int animationId, CancellationToken cancellationToken)
		{
			...
			foreach (AnimationChannel channel in animation.Channels)
			{
				...
				switch (path)
				{
					...
					default:
						GLTFSceneImporter.Debug.Log(LogType.Warning, $"Cannot read GLTF animation path (File: {_gltfFileName})");
						break;
				} // switch target type
			} // foreach channel
