...
namespace GLTF
{
	public static class GLTFHelpers
	{
		...
		public static void RemoveUndefinedReferences(GLTFRoot root)
		{
			...
			if (root.Animations != null)
			{
				foreach (GLTFAnimation animation in root.Animations)
				{
					...
					{
						foreach (GLTF.Schema.AnimationSampler animationSampler in animation.Samplers)
						{
		...
		private static unsafe void ApplySparseAccessorsVec3(ref NumericArray resultArray, AttributeAccessor attributeAccessor, AttributeAccessor sparseValues, AttributeAccessor sparseIndices)
		{
			...
			Accessor.AsSparseFloat3Array(attributeAccessor.AccessorId.Value, ref sparseArrays[0], bufferViewCache1.GetUnsafePtrAlt(), 0);
			Accessor.AsSparseUIntArray(attributeAccessor.AccessorId.Value, ref sparseArrays[1], bufferViewCache2.GetUnsafePtrAlt(), 0);
		...
		/// </summary>
		/// <param name="attributes">A dictionary that contains a mapping of attribute name to data needed to parse</param>
		public static unsafe void BuildMeshAttributes(ref Dictionary<string, AttributeAccessor> attributes,ref Dictionary<string, (AttributeAccessor sparseIndices, AttributeAccessor sparseValues)> sparseAccessors)
		{
			if (attributes.TryGetValue(SemanticProperties.POSITION, out var attributeAccessor))
			{
				...
				attributeAccessor.AccessorId.Value.AsVertexArray(ref resultArray, bufferViewCache.GetUnsafePtrAlt(), 0, attributeAccessor.AccessorId.Value.Normalized);
			...
			if (attributes.TryGetValue(SemanticProperties.INDICES, out var attributeAccessorIndices))
			{
				...
				attributeAccessorIndices.AccessorId.Value.AsTriangles(ref resultArray, bufferViewCache.GetUnsafePtrAlt());
			...
			if (attributes.TryGetValue(SemanticProperties.NORMAL, out var attributeAccessorNormals))
			{
				...
				attributeAccessorNormals.AccessorId.Value.AsNormalArray(ref resultArray, bufferViewCache.GetUnsafePtrAlt(), 0, attributeAccessorNormals.AccessorId.Value.Normalized);
			...
			for (int i = 0; i < SemanticProperties.TexCoord.Length; i++)
			{
				if (attributes.TryGetValue(SemanticProperties.TexCoord[i], out var attributeAccessorTexCoord))
				{
					...
					attributeAccessorTexCoord.AccessorId.Value.AsTexcoordArray(ref resultArray, bufferViewCache.GetUnsafePtrAlt(), 0, attributeAccessorTexCoord.AccessorId.Value.Normalized);
			...
			if (attributes.TryGetValue(SemanticProperties.TANGENT, out var attributeAccessorTangent))
			{
				...
				attributeAccessorTangent.AccessorId.Value.AsTangentArray(ref resultArray, bufferViewCache.GetUnsafePtrAlt(), 0, attributeAccessorTangent.AccessorId.Value.Normalized);
			...
			for (int i = 0; i < SemanticProperties.Weight.Length; i++)
			{
				if (attributes.TryGetValue(SemanticProperties.Weight[i], out var attributeAccessorWeight))
				{
					...
					attributeAccessorWeight.AccessorId.Value.AsFloat4Array(ref resultArray, bufferViewCache.GetUnsafePtrAlt(), 0, attributeAccessorWeight.AccessorId.Value.Normalized);
			...
			for (int i = 0; i < SemanticProperties.Joint.Length; i++)
			{
				if (attributes.TryGetValue(SemanticProperties.Joint[0], out var attributeAccessorJoint))
				{
					...
					attributeAccessorJoint.AccessorId.Value.AsFloat4Array(ref resultArray, bufferViewCache.GetUnsafePtrAlt(), 0, attributeAccessorJoint.AccessorId.Value.Normalized);
					attributeAccessorJoint.AccessorContent = resultArray;
				}
			}
		}

		public static unsafe void BuildTargetAttributes(ref Dictionary<string, AttributeAccessor> attributes, float importScale = 1f)
		{
				...
				switch (kvp.Key)
				{
					case SemanticProperties.POSITION:
						if (hasScale)
						{
							...
							attributeAccessor.AccessorId.Value.AsFloat3ArrayConversion(ref resultArray, bufferPointer, conversionScale, 0, normalize);
						}
						else
							attributeAccessor.AccessorId.Value.AsFloat3Array(ref resultArray, bufferPointer, 0, normalize);
						break;
		...
		public static unsafe void BuildAnimationSamplers(ref Dictionary<string, List<AttributeAccessor>> samplers)
		{
			foreach (var samplerSet in samplers)
			{
				foreach (var attributeAccessor in samplerSet.Value)
				{
					...
					{
						case "time":
							attributeAccessor.AccessorId.Value.AsFloatArray(ref resultArray, bufferPointer, 0, attributeAccessor.AccessorId.Value.Normalized);
							break;
						case "translation":
						case "scale":
							attributeAccessor.AccessorId.Value.AsFloat3Array(ref resultArray, bufferPointer, 0, attributeAccessor.AccessorId.Value.Normalized);
							break;
						case "rotation":
							attributeAccessor.AccessorId.Value.AsFloat4Array(ref resultArray, bufferPointer, 0, attributeAccessor.AccessorId.Value.Normalized);
							break;
						case "weights":
							attributeAccessor.AccessorId.Value.AsFloatArray(ref resultArray, bufferPointer, 0, attributeAccessor.AccessorId.Value.Normalized);
							break;
		...
		private static void LoadBufferView(AttributeAccessor attributeAccessor, out byte[] bufferViewCache)
		{
			...
			var arr = new Il2CppStructArray<byte>(bufferViewCacheNative.Length);
			bufferViewCacheNative.CopyTo(arr);
			bufferViewCache = arr.ToArray();
		}
		...
		private static void MergeAnimationsAndSkins(GLTFRoot mergeToRoot, GLTFRoot mergeFromRoot, PreviousGLTFSizes previousGLTFSizes)
		{
			...
			if (mergeFromRoot.Animations != null)
			{
				...
				for (int i = previousGLTFSizes.PreviousAnimationCount; i < mergeToRoot.Animations.Count; ++i)
				{
					...
					foreach (GLTF.Schema.AnimationSampler sampler in animation.Samplers)
					{
