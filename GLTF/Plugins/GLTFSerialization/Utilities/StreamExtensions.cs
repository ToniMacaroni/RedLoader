	...
	internal static class StreamExtensions
	{
		...
		public static void CopyToSelf(this Stream source, int destinationOffset, uint amountToCopy)
		{
			...
			try
			{
				int read;
				int amountToRead;
				while ((source.Position = initialOffset + amountToCopy - (amountToRead = (int)System.Math.Min(_CopyBuffer.Length, amountToCopy))) >= 0 && (read = source.Read(_CopyBuffer, 0, amountToRead)) != 0)
				{
					if (read > highwaterMark) highwaterMark = read;
