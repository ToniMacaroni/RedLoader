...
namespace SFLoader
{
    public sealed class MelonAssembly
    {
        ...
        {
            UnregisterMelons("SFLoader is deinitializing.", true);
        }
                ...
                if (info.SystemType.IsSubclassOf(typeof(MelonPlugin)))
                {
                    ...
                    if (melon != null)
                    {
                        ...
                        if (!SemVersion.TryParse(info.Version, out _))
                            MelonLogger.Warning($"==Normal users can ignore this warning==\nMelon '{info.Name}' by '{info.Author}' has version '{info.Version}' which does not use the Semantic Versioning format. Versions using formats other than the Semantic Versioning format will not be supported in the future versions of SFLoader.\nFor more details, see: https://semver.org");
                    }
                    else
                    {
                        MelonLogger.Warning($"Skipping ({info.Name}) since it is written for the original SFLoader.");
                    }
                }
            }
