...
internal class SyntaxValidator : SyntaxVisitor
{
    ...
    public override void Visit(KeyValueSyntax keyValue)
    {
        ...
        switch (keyValue.Value)
        {
            ...
            case StringValueSyntax _:
                ...
                break;
            default:
                _diagnostics.Error(keyValue.Span,
                    keyValue.Value == null
                        ? "A KeyValueSyntax must have a non null Value"
                        : $"Not supported type `{keyValue.Value.Kind}` for the value of a KeyValueSyntax");
                return;
        }
    ...
    public override void Visit(ArraySyntax array)
    {
        ...
        for (var i = 0; i < items.ChildrenCount; i++)
        {
            ...
            {
                _diagnostics.Error(value.Span,
                    $"The array item of type `{value.Kind.ToString().ToLowerInvariant()}` doesn't match the type of the first item: `{firstKind.ToString().ToLowerInvariant()}`");
            }
    ...
    private void VerifyTable(TableSyntaxBase table)
    {
        ...
        {
            _diagnostics.Error(table.Span,
                $"The table{(isTableArray ? " array" : string.Empty)} must have an {table.OpenTokenKind} `{table.OpenTokenKind.ToText()}`");
        }

        if (table.CloseBracket == null)
        {
            _diagnostics.Error(table.Span,
                $"The table{(isTableArray ? " array" : string.Empty)} must have an {table.CloseTokenKind} `{table.CloseTokenKind.ToText()}`");
        }

        if (table.EndOfLineToken == null && table.Items.ChildrenCount > 0)
        {
            _diagnostics.Error(table.Span,
                $"The table{(isTableArray ? " array" : string.Empty)} must have a EndOfLine set after the open/closing brackets and before any elements");
        }
    ...
    private bool KeyNameToObjectPath(KeySyntax key, ObjectKind kind)
    {
        ...
        for (var i = 0; i < items.ChildrenCount; i++)
        {
            ...
            if (string.IsNullOrEmpty(dotItem))
            {
                return false;
            }
