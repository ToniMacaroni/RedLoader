...
public sealed class DateTimeValueSyntax : ValueSyntax
{
    ...
    {
        get => _token;
        set => ParentToThis(ref _token, value, value != null && value.TokenKind.IsDateTime(),
            $"The token kind `{value?.TokenKind}` is not a datetime token");
    }
    ...
    internal static string ToString(SyntaxKind kind, DateTime value)
    {
        switch (kind)
        {
            case SyntaxKind.OffsetDateTime:
                ...
                {
                    return time.ToString("yyyy-MM-dd'T'HH:mm:ssK", CultureInfo.InvariantCulture);
                }
