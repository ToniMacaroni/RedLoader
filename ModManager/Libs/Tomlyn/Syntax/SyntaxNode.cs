...
public abstract class SyntaxNode : SyntaxNodeBase
{
    ...
    public SyntaxNode GetChildren(int index)
    {
        ...
        {
            throw ThrowHelper.GetIndexArgumentOutOfRangeException(ChildrenCount);
        }
    ...
    {
        if (node != null && !expectedKindSuccess)
        {
            throw new InvalidOperationException($"Unexpected node kind `{node.TokenKind}` while expecting `{expectedMessage}`");
        }
    ...
    protected void ParentToThis<TSyntaxNode>(ref TSyntaxNode set, TSyntaxNode node, TokenKind expectedKind1, TokenKind expectedKind2)
        where TSyntaxNode : SyntaxToken
    {
        ParentToThis(ref set, node, node.TokenKind == expectedKind1 || node.TokenKind == expectedKind2,
            new ExpectedTuple2<TokenKind, TokenKind>(expectedKind1, expectedKind2));
    }
