...
internal class Parser<TSourceView> where TSourceView : ISourceView
{
    ...
    private InlineTableSyntax ParseInlineTable()
    {
        ...
        try
        {
            ...
            var expectingEndOfInitializer = false;
            while (true)
            {
                ...
                {
                    LogError(
                        $"Unexpected token `{_token.Kind}` while parsing inline table. Expecting a bare key or string instead of `{ToPrintable(_token)}`");
                    break;
                }
            }
        }
    ...
    private TableSyntaxBase ParseTableOrTableArray()
    {
        ...
        var isTableArray = _token.Kind == TokenKind.OpenBracketDouble;
    ...
    private SyntaxToken EatToken(TokenKind tokenKind)
    {
        ...
        else
        {
            ...
            var expectingTokenText = tokenText != null
                ? $"while expecting `{tokenText}` (token: `{tokenKind.ToString().ToLowerInvariant()}`)"
                : $"while expecting token `{tokenKind.ToString().ToLowerInvariant()}`";
            ...
            {
                LogError(
                    $"Unexpected token found `{ToPrintable(_token)}` (token: `{_token.Kind.ToString().ToLowerInvariant()}`) {expectingTokenText}");
            }
        }
    ...
    private void NextToken()
    {
        ...
        while ((result = _lexer.MoveNext()) && IsHidden(_lexer.Token.Kind))
        {
            ...
            {
                Span = new SourceSpan(_lexer.Source.SourcePath, _lexer.Token.Start, _lexer.Token.End), Kind = _lexer.Token.Kind,
                Text = _lexer.Token.GetText(_lexer.Source)
            });
        }
