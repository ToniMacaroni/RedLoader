...
namespace RedLoader.IL2CPP.RuntimeFixes;
...
internal static class RedirectStdErrFix
{
    ...
    public static void Apply()
    {
        ...
        {
            var errorFile = CreateFile(Path.Combine(LoaderEnvironment.LoaderDirectory, "ErrorLog.log"), GENERIC_WRITE,
                                       FILE_SHARE_READ,
            ...
            if (errorFile == INVALID_HANDLE_VALUE)
            {
                ...
                RLog.Warning("Failed to open error log file; skipping error redirection");
                return;
            }
            ...
            if (!SetStdHandle(STD_ERROR_HANDLE, errorFile))
                ...
                RLog.Warning("Failed to redirect stderr; skipping error redirection");
        }
