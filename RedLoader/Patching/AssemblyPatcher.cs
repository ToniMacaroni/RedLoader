...
public class AssemblyPatcher : IDisposable
{
    ...
    {
        DumpedAssembliesPath = Utility.CombinePaths(LoaderEnvironment.LoaderDirectory, "DumpedAssemblies", LoaderEnvironment.GameExecutableName)
    };
    ...
    private PatcherPluginMetadata ToPatcherPlugin(TypeDefinition type, string assemblyPath)
    {
        ...
        if (metadata == null)
        {
            ...
            RLog.Warning($"Skipping over type [{type.FullName}] as no metadata attribute is specified");
            return null;
        }
        ...
        if (string.IsNullOrEmpty(metadata.GUID) || !allowedGuidRegex.IsMatch(metadata.GUID))
        {
            ...
            RLog.Warning($"Skipping type [{type.FullName}] because its GUID [{metadata.GUID}] is of an illegal format");
            return null;
        }
        ...
        if (metadata.Version == null)
        {
            ...
            RLog.Warning($"Skipping type [{type.FullName}] because its version is invalid");
            return null;
        }
        ...
        if (metadata.Name == null)
        {
            ...
            RLog.Warning($"Skipping type [{type.FullName}] because its name is null");
            return null;
        }
        ...
        foreach (var keyValuePair in patchers)
        {
            ...
            foreach (var patcherPlugin in patcherCollection)
                ...
                catch (Exception e)
                {
                    ...
                    RLog.Error($"Failed to load patchers from type [{patcherPlugin.TypeName}]: {(e is ReflectionTypeLoadException re ? TypeLoader.TypeLoadExceptionToString(re) : e.ToString())}");
                }
            ...
            RLog.Msg($"Loaded {patcherCollection.Count} patcher type{(patcherCollection.Count == 1 ? "" : "s")} from [{assName.Name} {assName.Version}]");
        }
    ...
    public void PatchAndLoad()
    {
        ...
        foreach (var assemblyPatcher in PatcherPluginsSafe)
            ...
            catch (Exception ex)
            {
                ...
                RLog.Error($"Failed to run initializer of {assemblyPatcher.Info.GUID}: {ex}");
            }
        ...
        RLog.Msg($"Executing {PatcherContext.PatchDefinitions.Count} patch(es)");
        ...
        foreach (var patchDefinition in PatcherContext.PatchDefinitions.ToList())
        {
            ...
            bool RunPatcher(AssemblyDefinition assembly, string targetDll)
            {
                try
                {
                    ...
                    if (!isAssemblyPatch)
                    {
                        ...
                        if (targetType == null)
                        {
                            ...
                            RLog.Warning($"Unable to find type [{patchDefinition.TargetType.TargetType}] defined in {patchDefinition.MethodInfo.Name}. Skipping patcher");
                            return false;
                        }
                ...
                catch (Exception e)
                {
                    ...
                    RLog.Error($"Failed to run [{patchDefinition.FullName}] when patching [{assembly.Name.Name}]. This assembly will not be patched. Error: {e}");
        ...
        if (earlyLoadAssemblies.Count != 0)
            RLog.Warning(new StringBuilder()
        ...
        foreach (var kv in assemblies)
        {
            ...
            if (patchedAssemblies.Contains(filename))
            {
                ...
                RLog.Debug($"Loaded '{assembly.FullName}' into memory");
            }
        ...
        foreach (var assemblyPatcher in PatcherPluginsSafe)
            ...
            catch (Exception ex)
            {
                ...
                RLog.Error($"Failed to run finalizer of {assemblyPatcher.Info.GUID}: {ex}");
            }
    }
