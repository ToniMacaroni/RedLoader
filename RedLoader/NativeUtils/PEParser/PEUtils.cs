...
namespace RedLoader.NativeUtils.PEParser
{
    public static class PEUtils
    {
        ...
        {
            //RedLoader.MelonLogger.Msg("[moduleBaseAddress+0x0]: " + (*(byte*)(moduleBaseAddress + 0x0)));
        ...
        public static unsafe IntPtr GetExportedFunctionPointerForModule(long moduleBaseAddress, string importName)
        {
            ...
            for (uint i = 0; i < imageDirectoryEntryExport->size / sizeof(ImageExportDirectory); ++i)
            {
                ...
                //RedLoader.MelonLogger.Msg("pExportDirectoryI->name: " + string.Format("{0:X}", pExportDirectoryI->name));
                if (pExportDirectoryI->name != 0)
                {
                    ...
                    //RedLoader.MelonLogger.Msg("imagename: " + imagename);
                    /*
                    ...
                    */
                    ...
                    for (int j = 0; j < pExportDirectoryI->numberOfNames; ++j)
                    {
                        ...
                        //RedLoader.MelonLogger.Msg($"{imagename}::{importname} @ 0x{((ulong)functionaddress - (ulong)moduleBaseAddress):X} (0x{functionaddress:X} - 0x{moduleBaseAddress:X})");
