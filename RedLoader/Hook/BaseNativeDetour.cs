...
internal abstract class BaseNativeDetour<T> : INativeDetour where T : BaseNativeDetour<T>
{
    ...
    public void Apply()
    {
        ...
        RLog.Debug($"Original: {OriginalMethodPtr:X}, Trampoline: {TrampolinePtr:X}, diff: {Math.Abs(OriginalMethodPtr - TrampolinePtr):X}");
    ...
    private void Prepare()
    {
        ...
        RLog.Debug($"Preparing detour from 0x{OriginalMethodPtr:X2} to 0x{DetourMethodPtr:X2}");
        PrepareImpl();
        // Logger.LogDebug($"Prepared detour; Trampoline: 0x{TrampolinePtr:X2}");
        RLog.Debug($"Prepared detour; Trampoline: 0x{TrampolinePtr:X2}");
        IsPrepared = true;
    }
